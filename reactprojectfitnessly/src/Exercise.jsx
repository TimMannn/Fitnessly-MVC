...
const Exercise = () => {
  ...
  const handleEdit = (ID) => {
    ...
    handleShowEdit();
    axios
      .get(`https://localhost:7187/api/Exercise/exercise/${ID}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleDelete = (ID) => {
    ...
    if (window.confirm("Are you sure you want to delete this exercise?")) {
      axios
        .delete(`https://localhost:7187/api/Exercise/${ID}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    axios
      ...
      .catch((error) => {
        ...
        const errorMessages = error.response?.data?.messages || [
          error.response?.data?.message || "Error updating exercise",
        ];
  ...
  const handleSave = () => {
    ...
    axios
      ...
      .catch((error) => {
        const errorMessages = error.response?.data?.messages || [
          error.response?.data?.message || "Error adding exercise",
        ];
        errorMessages.forEach((msg) => toast.error(msg));
      });
  };
